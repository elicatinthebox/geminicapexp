...
*Limitations*: Since this is a demonstration project I tried to limit the comparison by not repeating the application of the same functions on both apartments equally, this is because the main purpose is to show the capabilities of Gemini. For the same reason and to save costs the data samples have been limited. Keep in mind that in a real use case the data could be more substantial and require guarantees on their protection. For an in-depth analysis on real data I strongly recommend including all the documents of the context in a fair manner for all the properties to be compared and since it is a financial real estate analysis to always have the report produced supervised by a professional in the sector authorized to practice the profession in the geographical area of ​​jurisprudence in which you intend to operate.
